...
public class App_Inbound_KIASEPOS_Test extends MyAutomationConstants {
	...
	//		System.out.println("hiiiiiiiiiiiiiiiiiiiiiii  test"); 
			...
			//pr change          for (int i = 0; i < name.length; i++) 
			//pr change         {
			//pr change          	 filefound = false;
			//pr change           for (int j = 0; j < fileList.size(); j++) 
			//pr change           {
			//pr change             ChannelSftp.LsEntry lsEntry = (ChannelSftp.LsEntry) fileList.get(j);
			//pr change           name11 = (lsEntry.getFilename()); 
			//                if (lsEntry.getFilename()).contains(name[i]) { 
			//pr change            if (name11.contains(name[i])) { 
			//pr change            	 filefound = true;
			//pr change           	 k=i+1;
			//pr change         	 System.out.println("The file [" + k +"]:" + name[i] + " is found in the input folder");
			//pr change         	}
